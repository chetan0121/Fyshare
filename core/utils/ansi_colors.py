...
class ANSI_Manager:
    ...
    def _to_escape(codes: list[str]):
        ...
        if not codes:
            return ""
        return f"{ANSI_Manager.ESC}{';'.join(str(c) for c in codes)}m"
    ...
    def styled(text: str, *styles) -> str:
        ...
        resolved = ANSI_Manager._resolve(styles)
        escape_seq = ANSI_Manager._to_escape(resolved)

        return f"{escape_seq}{text}{ANSI_Manager.RESET}"
    ...
    def print_custom(txt="", *codes, prefix="", end="\n"):
        ...
        """
        msg = ANSI_Manager.styled(txt, *codes)
